...
@customElement('vaadin-components')
export default class VaadinComponents extends LitElement {

  @property() validating = false;
  @property() error = false;
  @property() errorMessage = '';
  @property() submitting = false;
  @property() pristine = false;
  ...
  render() {
    ...
    return html`
      ...
      <form autocomplete="off">
        ...
        <div>
          <vaadin-text-field theme="no-animation" label="Username"
            .errorMessage=${this.errorMessage}
            .invalid=${this.error && field.touched}
            @change=${this.onChange}>
            ${this.validating ? html`<div slot="suffix" class="spinner">validating...</div>` : ``}
          </vaadin-text-field>
          ${this.error ? '' : html`<div class="spacer"></div>`}
        </div>

        <vaadin-button theme="primary" @click=${form.submit}
          ?disabled=${this.pristine || this.submitting || this.error || this.validating}
        >
          Submit
        </vaadin-button>
      </form>
